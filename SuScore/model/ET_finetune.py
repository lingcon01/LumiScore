...
class GenScore(nn.Module):
    def __init__(self, ligand_model, target_model, in_channels, hidden_dim, n_gaussians, dropout_rate=0.15,
        ...
        self.ionic_coeff = nn.Parameter(th.tensor([1.0]))
        ...
        self.interaction_net = nn.ModuleList(
            [InteractionNet(self.num_filter, hidden_dim)
             for _ in range(2)])
        ...
        nn.init.uniform_(self.distance[0].weight, a=0.1, b=0.4)
        nn.init.uniform_(self.distance[0].bias, a=0.1, b=0.4)

        print(self.distance[0].weight)
        print(self.distance[0].bias)
    ...
    def ionic_hbond(self, dm, h, vdw_radius1, vdw_radius2, A, charge1, charge2):
        ...
        retval = dm * A
        retval = retval.clamp(min=0.0, max=1.0)
        coeff = self.ionic_coeff * self.ionic_coeff
        retval = retval * coeff * charge1 * charge2
        retval = retval.sum(-1).sum(-1).unsqueeze(-1)
        return retval
    ...
    def interact(self, dm, h1, h2, dropout):
        edge = dm.unsqueeze(-1).repeat(1, 1, 1, self.filter_center.size(-1))

        filter_center = (self.filter_center.unsqueeze(0).unsqueeze(0).unsqueeze(0)).to(dm.device)

        edge = th.exp(-th.pow(edge - filter_center, 2) * self.filter_gamma)
        edge = edge.detach()
        ...
        for i in range(len(self.interaction_net)):
            ...
            new_h2 = self.interaction_net[i](h2, h1,
                                             ...
                                             adj12.permute(0, 2, 1))
            h1, h2 = new_h1, new_h2
            h1 = F.dropout(h1, training=dropout, p=self.dropout_rate)
            h2 = F.dropout(h2, training=dropout, p=self.dropout_rate)
    ...
    def forward(self, data_ligand, data_target, train):
        ...
        h_l_x, h_t_x = self.interact(dm, h_l_x, h_t_x, train)
        ...
        vdw_energy, vdw_radius = self.cal_vdw_interaction(dm, C, radius1, radius2, physic_ligand[:, :, 1].squeeze(-1), physic_target[:, :, 1].squeeze(-1))

        retval.append(vdw_energy)

        # hbond

        retval.append(self.vina_hbond(dm, C, radius1, radius2, A_hbond))

        # metal complex

        retval.append(self.vina_hbond(dm, C, radius1, radius2, A_metal))

        # hydrophobic

        retval.append(self.vina_hydrophobic(dm, C, radius1, radius2, A_hydro))

        # retval.append(self.cal_coul(dm, C, physic_ligand[:, :, 6], physic_target[:, :, 6]))

        retval = th.cat(retval, -1)
        # print(f"retval is {retval}")

        penalty = 1 + self.rotor_coeff * self.rotor_coeff * data_ligand.rotor
        ...
        return vdw, vdw_radius, retval.float()
