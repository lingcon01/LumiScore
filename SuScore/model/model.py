...
class GraphTransformer(nn.Module):
    ...
    def forward(self, data):
        ...
        edge_feats = self.edge_encoder(data.edge_attr[:, :10])
...
class SubGT(nn.Module):
    ...
    def forward(self, data, model_type=None):
        ...
        'combined_subgraphs'], sample['hop_indicator'], sample['num_nodes'] = self.transform(data.edge_index, data.x.size()[0])
...
class GenScore(nn.Module):
    ...
    def forward(self, data_ligand, data_target, train=None):
