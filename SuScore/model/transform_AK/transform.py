...
class SubgraphsTransform(object):
    ...
    def __call__(self, adj, num_nodes):
        ...
        # a = torch.nonzero(torch.tensor(adj), as_tuple=False)
        # b = a[a[:, 0] != a[:, 1]]
        # adj1 = b.transpose(1, 0)
        # adj = torch.stack(adj).to('cuda')
        ...
        hop_indicator = hop_indicator
        num_nodes = num_nodes  # set number of nodes of the current graph
