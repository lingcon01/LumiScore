...
if __name__ == '__main__':
    ...
    p.add_argument('--val_dir3', type=str, default="/home/suqun/tmp/GMP/pretrain/GenScore/feats/derivate")
    ...
    train_dataset2 = PDBbindDataset(ids="%s/%s_ids6.npy" % (datadir, 'fep'),
                                    ligs="%s/%s_lig.pt" % (datadir, 'fep'),
                                    prots="%s/%s_prot.pt" % (datadir, 'fep')
                                    )
    ...
    train_inds2, _ = train_dataset2.train_and_test_split(valnum=args.valnum, seed=args.seeds)
    ...
    train_data2 = PDBbindDataset(ids=train_dataset2.pdbids[train_inds2],
                                 ligs=train_dataset2.gls[train_inds2],
                                 prots=train_dataset2.gps[train_inds2],
                                 labels=train_dataset2.labels[train_inds2]
                                 )
    ...
    for epoch in range(args.num_epochs):
        ...
        fep_spearman, fep_pr, derivate_spearman, derivate_pr, fep_rmse, derivate_rmse = fep_score(model, args.seeds)

        early_stop = stopper.step({"fep": fep_pr, "derivate": derivate_pr}, model)

        # if fep_pr > best_fep_pr:
        #     th.save(model.state_dict(), '/home/suqun/tmp/GMP/pretrain/EGMDN/ET_finetune/SuScore_fep_pr_mdn_v1_local.pth')
        #     best_fep_pr = fep_pr

        # if derivate_pr > best_derivate_pr:
        #     th.save(model.state_dict(), '/home/suqun/tmp/GMP/pretrain/EGMDN/ET_finetune/SuScore_derivate_pr_mdn_v1_local.pth')
        #     best_derivate_pr = derivate_pr


        try:
            logging.info(
                'epoch {:d}/{:d}, train_loss {:.4f}, total_loss_val1 {:.4f}, affi_loss_val1 {:.4f}, fep_spearman {:.4f}, fep_pr {:.4f}, fep_rmse {:.4f}, derivate_spearman {:.4f}, derivate_pr {:.4f}, derivate_rmse {:.4f}, best fep validation {:.4f}, best derivate validation {:.4f}'.format(
                    epoch + 1, args.num_epochs, total_loss_train, total_loss_test1, test_pr1, fep_spearman, fep_pr, fep_rmse,
                    derivate_spearman, derivate_pr, derivate_rmse,
                    stopper.best_score["fep"], stopper.best_score["derivate"]))
        except:
            pass
